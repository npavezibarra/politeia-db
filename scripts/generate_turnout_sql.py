...
def main():
    ...
    # Merge by election_id or matching key
...
VALUES
    ({int(row['wp_election_id'])}, {int(row['valid_votes'])}, {int(row['blank_votes'])},
     {int(row['null_votes'])}, {int(row['padron'])}, {int(row['votantes'])},
     {float(row['participation_pct'])}, 'https://www.emol.com/especiales/2024/nacional/elecciones/municipales/')
